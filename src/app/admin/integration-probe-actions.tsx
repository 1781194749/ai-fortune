...
export function AdminIntegrationProbeActions({ adminToken }: { adminToken?: string }) {
  ...
  async function runProbe() {
    setLoading(true);
    setOk(null);
    setMessage("正在诊断第三方集成...");

    const response = await fetch(adminApiPath("/api/admin/integrations/probe", adminToken), {
      method: "POST",
    ...
    if (!response.ok || !data.ok) {
      setOk(false);
      setMessage(data.message ?? "第三方诊断存在阻断项。");
      router.refresh();
      return;
    }

    setOk(true);
    setMessage(data.message ?? "第三方诊断完成。");
    router.refresh();
  }
