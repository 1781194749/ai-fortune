...
export function getProductionHealthChecks(env: Env = process.env) {
  ...
  return [
    check({
      ...
      action: "配置正式 HTTPS 域名，并用于支付回调、邮件链接和 SEO。",
    }),
    check({
      id: "auth-secret",
      group: "基础配置",
    ...
    check({
      ...
      action: "配置生产 PostgreSQL，并执行 Prisma 迁移或 npm run prisma:push。",
    }),
    check({
      id: "admin",
      group: "基础配置",
      ...
      detail:
        env.ADMIN_DASHBOARD_ENABLED === "true"
          ? present(env, "ADMIN_ACCESS_TOKEN")
            ...
            : "已开启但缺 token"
          : "未开启",
      action: "生产环境启用后台时必须设置 ADMIN_ACCESS_TOKEN；不开启则 /admin 返回 404。",
    }),
    check({
      id: "openai",
      group: "AI 能力",
      label: "OpenAI API Key",
      status: features.openaiConfigured ? "ready" : "warning",
      detail: features.openaiConfigured ? "已配置" : "未配置，将使用本地降级回答",
    ...
    check({
      ...
      group: "AI 能力",
      label: "手相视觉模型",
      status: features.openaiConfigured && visionModel ? "ready" : "warning",
      detail: features.openaiConfigured
        ? visionModel
          ? `使用 ${visionModel}`
          : "未配置 OPENAI_VISION_MODEL，将回退默认模型"
        : "未配置 OpenAI，将使用本地降级手相分析",
    ...
    check({
      ...
      group: "支付",
      label: "支付模式",
      status: livePaymentEnabled ? "ready" : "blocking",
      detail: `当前 PAYMENT_PROVIDER=${features.paymentProvider}`,
      action: "正式收费前切换 PAYMENT_PROVIDER=live，并完成支付宝/微信支付至少一个渠道联调。",
    }),
    check({
      id: "payment-channel",
      group: "支付",
      label: "真实支付渠道",
      status: anyLivePaymentReady ? "ready" : "blocking",
      detail: anyLivePaymentReady
        ? [alipayReady ? "支付宝已就绪" : undefined, wechatPayReady ? "微信支付已就绪" : undefined]
            .filter(Boolean)
            .join("，")
        : "尚无可用于真实收款的完整支付渠道",
      action: "至少完成支付宝或微信支付其中一个渠道的商户参数、签名诊断和小额订单验收。",
    }),
    check({
      id: "alipay",
      group: "支付",
    ...
    check({
      ...
      detail:
        env.AUTH_WECHAT_ENABLED === "true"
          ? "已开启"
          : "未开启，当前邮箱登录可用",
      action: "微信开放平台资质完成后配置 App ID 和 App Secret。",
    }),
    check({
      id: "compliance",
      group: "合规",
